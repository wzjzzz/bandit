...
class cube:
    def __init__(self, x, ys, xs, cube_length, d=2, reward=0):
        '''
        Parameters
        ----------
...
#         denominator = np.sqrt(np.log(self.T/self.epsilon))
    ...
    def sample(self, cube):
        return np.random.uniform(cube.x, cube.x+cube.cube_length)
        
    
    def get_reward(self, arm):
        '''  get function value of arm
        function: GOLDSTEIN-PRICE FUNCTION  x1, x2 in [0, 1]
        arm: shape (2, 1) in this example
        '''
        x1, x2 = arm[0], arm[1]
        x1, x2 = 4*x1 - 2, 4*x2 - 2
        v1 = 1 + (x1 + x2 + 1)**2 * \
